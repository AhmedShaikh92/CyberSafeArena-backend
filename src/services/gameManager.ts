...
export interface GameBroadcaster {
  ...
  onActionsUpdate: (
    gameId: string,
    actions: { attacker: unknown[]; defender: unknown[] },
  ) => void;
}
...
export class GameManager {
  ...
  public freeUser(userId: string): void {
  ...
  public enqueue(entry: IMatchmakingEntry): {
    queued: boolean;
    gameId?: string;
  } {
    this.dequeue(entry.userId);

    if (this.userGameMap.has(entry.userId)) {
      const staleGameId = this.userGameMap.get(entry.userId)!;
      const existingGame = this.activeGames.get(staleGameId);

      if (!existingGame) {
        // Game already cleaned up — stale map entry
        console.warn(
          `[GameManager] enqueue: clearing stale userGameMap for ${entry.userId}`,
        );
        this.userGameMap.delete(entry.userId);
      } else if (existingGame.phase === "aar") {
        // Game is effectively over — free this user and clean up if other player already left
        console.warn(
          `[GameManager] enqueue: game ${staleGameId} is in AAR, freeing ${entry.userId}`,
        );
        this.userGameMap.delete(entry.userId);
        const otherPlayer =
          existingGame.redPlayer === entry.userId
            ? existingGame.bluePlayer
            : existingGame.redPlayer;
        if (!this.userGameMap.has(otherPlayer)) {
          this.activeGames.delete(staleGameId);
        }
      } else {
        // Game is genuinely active (lobby / briefing / active) — block re-queue
        console.warn(
          `[GameManager] enqueue blocked: user ${entry.userId} is in game ${staleGameId} (phase: ${existingGame.phase})`,
        );
        return { queued: false };
      }
    }
    ...
    if (opponentIndex !== -1) {
      ...
      const redPayload: IMatchFoundPayload = {
        ...
        role: "red_team",
        opponent: {
          userId: blueEntry.userId,
          username: blueEntry.username,
          level: blueEntry.level,
        },
      ...
      const bluePayload: IMatchFoundPayload = {
        ...
        role: "blue_team",
        opponent: {
          userId: redEntry.userId,
          username: redEntry.username,
          level: redEntry.level,
        },
        scenario: scenario.type,
        difficulty: scenario.difficulty as DifficultyType,
      };

      this.onMatchFound?.(game.gameId, redEntry, blueEntry, {
        red: redPayload,
        blue: bluePayload,
      });
  ...
  public startGame(gameId: string): boolean {
    ...
    if (!game || game.phase !== "lobby" || !game.redPlayer || !game.bluePlayer)
      return false;
    game.phase = "briefing";
  ...
  public leaveGame(userId: string): void {
    ...
    // Only force-end the game if it's not already in AAR/winding down
    // In AAR, let the game loop expire it naturally
    if (game.phase !== "aar") {
      this.endGame(gameId);
    }
  }
  ...
  public processAction(
    ...
    const result = game.engine.processAction(
      userId,
      {
        userId,
        type: actionType,
    ...
    this.broadcaster?.onActionProcessed(
      gameId,
      userId,
      actionType,
      result.success,
  ...
  public getGameState(gameId: string) {
    ...
    const timeRemaining =
      game.phase === "active"
        ? Math.max(0, config.game.roundDuration - phaseElapsed)
        : game.phase === "briefing"
          ? Math.max(0, config.game.briefingDuration - phaseElapsed)
  ...
  public getAllGames() {
    return Array.from(this.activeGames.values()).map((game) => ({
      ...
      bluePlayer: game.bluePlayer,
      scenario: {
        type: game.scenario.type,
        difficulty: game.scenario.difficulty,
      },
    }));
  }
  ...
  private startGameLoop(): void {
    this.gameUpdateInterval = setInterval(() => {
      for (const [gameId, game] of this.activeGames) {
        // Auto-expire games stuck in lobby for more than 30s —
        // this cleans up matches where both players disconnected before startGame
        if (
          game.phase === "lobby" &&
          Date.now() - game.phaseStartTime > 30_000
        ) {
          console.warn(`[GameManager] Expiring stale lobby game ${gameId}`);
          this.endGame(gameId);
          continue;
        }
        ...
        if (game.phase === "active") {
          ...
          for (const event of lastEvents) {
            if (
              ...
              tickStart - event.timestamp < 1500
            ) {
          ...
          const timeRemaining = Math.max(
            0,
            config.game.roundDuration - phaseElapsed,
          );
  ...
  private _transitionPhase(game: ActiveGame): boolean {
    ...
    switch (game.phase) {
      ...
      case "active":
        if (
          phaseElapsed >= config.game.roundDuration ||
          !game.engine.isActive()
        ) {
          game.phase = "aar";
          game.phaseStartTime = Date.now();
          game.engine.end();
  ...
  private _broadcastActionsUpdate(
    gameId: string,
    actions: { attacker: unknown[]; defender: unknown[] },
  ): void {
  ...
  private _buildAarResult(game: ActiveGame) {
    ...
    const redOutcome =
      winner === "red_player" ? "win" : winner === "draw" ? "draw" : "loss";
    const blueOutcome =
      winner === "blue_player" ? "win" : winner === "draw" ? "draw" : "loss";
    ...
    const timeline = state.events.slice(0, 20).map((e) => ({
      time: new Date(game.startTime + (e.timestamp ?? 0))
        .toISOString()
        .substr(14, 5),
      event: e.message,
      severity: e.severity,
    ...
    return {
        ...
        playerSummary: {
          summary:
            redOutcome === "win"
              ...
              : "Threat overcame defenses",
          actionsCount: fullAAR.redPlayerSummary.actions,
          correctActions: Math.round(
            (fullAAR.redPlayerSummary.successRate / 100) *
              fullAAR.redPlayerSummary.actions,
          ),
          avgResponseTime: Math.round(
            fullAAR.redPlayerSummary.timing.averageResponseTime / 1000,
          ),
        },
        performanceScores: {
          attackSuccess: Math.round(fullAAR.redPlayerSummary.successRate),
          speed:
            fullAAR.redPlayerSummary.timing.averageResponseTime < 10000
              ? 90
              : 50,
          coverage: Math.round(
            (fullAAR.redPlayerSummary.vulnerabilityAnalysis.exploited.length /
              2) *
              100,
          ),
        },
      ...
      blue: {
        ...
        playerSummary: {
          ...
          correctActions: Math.round(
            (fullAAR.bluePlayerSummary.successRate / 100) *
              fullAAR.bluePlayerSummary.actions,
          ),
          avgResponseTime: Math.round(
            fullAAR.bluePlayerSummary.timing.averageResponseTime / 1000,
          ),
        },
        performanceScores: {
          defenseSuccess: Math.round(fullAAR.bluePlayerSummary.successRate),
          responseSpeed:
            fullAAR.bluePlayerSummary.timing.averageResponseTime < 10000
              ? 90
              : 50,
          vulnerabilitiesPatchedPct: Math.round(
            (fullAAR.bluePlayerSummary.vulnerabilityAnalysis.identified.length /
              2) *
              100,
          ),
        },
  ...
  private _calcXP(score: number, won: boolean): number {
    ...
    const performance =
      score >= 80 ? 50 : score >= 60 ? 25 : score >= 40 ? 10 : 0;
  ...
  public _getActionsForScenario(scenario: IGameScenario) {
    const attackerActions: Record<IGameScenario["type"], unknown[]> = {
      brute_force: [
        {
          id: "atk_credential_stuff",
          label: "Cred Stuffing",
          icon: "🔑",
          description: "Use leaked credential DB",
          color: "#ff2244",
          cooldown: 12,
        },
        {
          ...
          label: "Distribute",
          icon: "🌐",
          description: "Spread across proxies",
          color: "#ff4400",
          cooldown: 8,
        },
        {
          ...
          label: "Brute Login",
          icon: "💥",
          description: "Hammer login portal",
          color: "#ff6600",
          cooldown: 6,
        },
        {
          id: "atk_bypass_lockout",
          label: "Bypass Lockout",
          icon: "🔓",
          description: "Circumvent lockout policy",
          color: "#ff2266",
          cooldown: 20,
        },
      ],
      sql_injection: [
        {
          ...
          label: "Inject Query",
          icon: "💉",
          description: "Exploit SQL endpoint",
          color: "#ff2244",
          cooldown: 15,
        },
        {
          id: "atk_bypass_filter",
          label: "Bypass Filter",
          icon: "🔍",
          description: "Try encoded payloads",
          color: "#ff4400",
          cooldown: 10,
        },
        {
          ...
          label: "Leak Schema",
          icon: "📄",
          description: "Force verbose DB errors",
          color: "#ff6600",
          cooldown: 12,
        },
        {
          ...
          label: "Blind Inject",
          icon: "👁",
          description: "Time-based blind extraction",
          color: "#ff2266",
          cooldown: 20,
        },
      ],
      xss: [
        ...
        },
        {
          ...
          label: "Bypass CSP",
          icon: "🛡",
          description: "Circumvent CSP headers",
          color: "#ff4400",
          cooldown: 15,
        },
        {
        ...
        {
          id: "atk_event_handler",
          label: "Event Handler",
          icon: "⚡",
          description: "Inject via event attrs",
          color: "#ff2266",
          cooldown: 8,
        },
      ],
      phishing: [
        {
          ...
          label: "Spoof Domain",
          icon: "📧",
          description: "Send spoofed email",
          color: "#ff2244",
          cooldown: 10,
        },
        {
          ...
          label: "Bypass Auth",
          icon: "🔓",
          description: "Bypass email auth checks",
          color: "#ff4400",
          cooldown: 15,
        },
        {
          ...
          label: "Fake Link",
          icon: "🔗",
          description: "Embed malicious URL",
          color: "#ff6600",
          cooldown: 8,
        },
        {
          ...
          label: "Clone Site",
          icon: "🪞",
          description: "Clone login portal",
          color: "#ff2266",
          cooldown: 20,
        },
      ],
      jwt_manipulation: [
        {
          ...
          label: "Forge Token",
          icon: "🎫",
          description: "Forge JWT signature",
          color: "#ff2244",
          cooldown: 15,
        },
        {
          ...
          label: "None Algo",
          icon: "🔑",
          description: "Use alg:none bypass",
          color: "#ff4400",
          cooldown: 10,
        },
        {
          ...
          label: "Crack Secret",
          icon: "💥",
          description: "Brute-force JWT secret",
          color: "#ff6600",
          cooldown: 20,
        },
        {
          ...
          label: "Claim Inject",
          icon: "👤",
          description: "Inject admin role claim",
          color: "#ff2266",
          cooldown: 12,
        },
      ],
      network_anomaly: [
        ...
        },
        {
          ...
          label: "DDoS Wave",
          icon: "🌊",
          description: "Flood target with traffic",
          color: "#ff4400",
          cooldown: 25,
        },
        {
          ...
          label: "Amplify",
          icon: "📶",
          description: "Amplify attack volume",
          color: "#ff6600",
          cooldown: 20,
        },
        {
          ...
          label: "Exfiltrate",
          icon: "📤",
          description: "Extract network data",
          color: "#ff2266",
          cooldown: 15,
        },
      ],
    ...
    const defenderActions: Record<IGameScenario["type"], unknown[]> = {
      brute_force: [
        {
          ...
          label: "Rate Limit",
          icon: "⏱",
          description: "Limit login attempts/min",
          color: "#00d4ff",
          cooldown: 8,
        },
        {
          ...
          label: "Block IP",
          icon: "🚫",
          description: "Blacklist attacker IP",
          color: "#0080ff",
          cooldown: 5,
        },
        {
        ...
        {
          id: "deploy_captcha",
          label: "Deploy CAPTCHA",
          icon: "🤖",
          description: "Block automated attempts",
          color: "#00ccaa",
          cooldown: 10,
        },
      ],
      sql_injection: [
        {
          id: "implement_parameterized_queries",
          label: "Param Queries",
          icon: "🔧",
        ...
        },
        {
          ...
          icon: "🧹",
          description: "Strip malicious chars",
          color: "#0080ff",
          cooldown: 8,
        },
        {
          ...
          label: "Hide Errors",
          icon: "🙈",
          description: "Suppress DB error msgs",
          color: "#00aaff",
          cooldown: 10,
        },
        {
          ...
          label: "DB Firewall",
          icon: "🛡",
          description: "Block suspicious queries",
          color: "#00ccaa",
          cooldown: 12,
        },
      ],
      xss: [
        {
          id: "implement_output_encoding",
          label: "Encode Output",
          icon: "🔧",
          description: "HTML-escape all output",
          color: "#00d4ff",
          cooldown: 10,
        },
        {
        ...
        },
        {
          ...
          label: "Safe DOM",
          icon: "🌐",
          description: "Use textContent not innerHTML",
          color: "#00ccaa",
          cooldown: 10,
        },
      ],
      phishing: [
        {
          id: "verify_sender_domain",
          label: "Verify Sender",
          icon: "✉️",
          description: "Check domain authenticity",
          color: "#00d4ff",
          cooldown: 5,
        },
        {
          ...
          label: "Check DMARC",
          icon: "🔍",
          description: "Verify SPF/DKIM/DMARC",
          color: "#0080ff",
          cooldown: 8,
        },
        {
          ...
          label: "Scan URLs",
          icon: "🔗",
          description: "Check links for malware",
          color: "#00aaff",
          cooldown: 6,
        },
        {
          ...
          label: "Block Domain",
          icon: "🚫",
          description: "Blacklist phishing domain",
          color: "#00ccaa",
          cooldown: 10,
        },
      ],
      jwt_manipulation: [
        ...
        },
        {
          ...
          label: "Verify Token",
          icon: "🎫",
          description: "Full token verification",
          color: "#0080ff",
          cooldown: 6,
        },
        {
        ...
        {
          id: "strengthen_algorithm",
          label: "Strong Algo",
          icon: "💪",
          description: "Enforce HS256 minimum",
          color: "#00ccaa",
          cooldown: 10,
        },
      ],
      network_anomaly: [
        {
          ...
          label: "Block IP",
          icon: "🚫",
          description: "Blacklist attacker IPs",
          color: "#00d4ff",
          cooldown: 5,
        },
        {
        ...
        },
        {
          ...
          label: "DDoS Shield",
          icon: "🛡",
          description: "Activate DDoS mitigation",
          color: "#00aaff",
          cooldown: 15,
        },
        {
          ...
          label: "Rate Limit",
          icon: "⏱",
          description: "Throttle suspicious traffic",
          color: "#00ccaa",
          cooldown: 8,
        },
      ],
